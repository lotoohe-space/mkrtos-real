...
struct bk_cache* occ_bk(dev_t dev_no,bk_no_t bk_no,void* addr){
    ...
    bk_ch->cache = addr;
    if (!MK_GET_BIT(bk_ch->flag, 2)) {
        if (bk_ops->read_bk(bk_no, bk_ch->cache) < 0) {
        }
        MK_SET_BIT(bk_ch->flag, 2);
    }
    MK_SET_BIT(bk_ch->flag,1);
    SET_MMAP(*bk_ch);
    unlock_bk(bk_ch);
    return bk_ch;
}
int32_t rel_bk1(dev_t dev_no,void* addr){
    ...
    for(i=0;i<cache_len;i++){
        ...
        if(
            ...
            if (MK_GET_BIT(bk_cache_ls[i].flag, 1)) {
                //先擦除
                ...
                MK_SET_BIT(bk_cache_ls[i].flag,0);
                MK_SET_BIT(bk_cache_ls[i].flag,1);
            }
...
void sync_bk(dev_t dev_no,uint32_t bk_no){
    ...
    lock_bk(sync_cache);
    if (MK_GET_BIT(sync_cache->flag, 1)) {
        //先擦除
        ...
        MK_SET_BIT(sync_cache->flag,0);
        MK_SET_BIT(sync_cache->flag,1);
    }
...
int32_t wbk(dev_t dev_no,uint32_t bk_no,uint8_t *data,uint32_t ofs,uint32_t size) {
    ...
    if(ofs==0 && size==bk_tmp->bk_size) {
        //如果刚好写一块，则没必要读
        MK_SET_BIT(bk_tmp->flag,2);
    }else {
        //否则读这一块
        if (!MK_GET_BIT(bk_tmp->flag, 2)) {
...
struct bk_cache* bk_read(dev_t dev_no,uint32_t bk_no
    ...
    if (!not_r && !MK_GET_BIT(bk_tmp->flag, 2)) {
        if (bk_ops->read_bk(bk_no, bk_tmp->cache) < 0) {
        }
