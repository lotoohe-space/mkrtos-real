...
struct bk_dev {
	...
	void *start_addr_in_mem; //!<在内存的起始地址
	//块数
	uint32_t bk_count;
	//块设备缓存
...
int32_t bk_dev_reg_param(dev_t major_no, uint32_t bk_count, uint32_t cache_len,
	...
	devs_bk[major_no].start_addr_in_mem = start_addr_in_mem;
	return 0;
}
...
int32_t get_bk_cn(dev_t major_no, uint32_t *bk_cn) {
	...
	*bk_cn = devs_bk[major_no].bk_count;
	return 0;
}
int32_t get_bk_start_addr_in_mem(dev_t major_no, uint32_t *addr) {
	...
	*addr = devs_bk[major_no].start_addr_in_mem;
	return 0;
}
/**
 * 取消注册字符设备
