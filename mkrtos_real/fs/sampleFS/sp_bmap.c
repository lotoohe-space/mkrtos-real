...
int32_t sp_set_bk_no_status(struct super_block *sb, uint32_t usedBkInx,
	...
	struct bk_cache* tmp_bk_cache;

	tmp_bk_cache = bk_read(sb->s_dev_no, sp_sb->bk_used_bk_st_inx + bkInx
			, 1, 0);
	rByte = tmp_bk_cache->cache[ROUND_DOWN(usedBkInx, 8) % sb->s_bk_size];
...
//	}
	...
	tmp_bk_cache->cache[ROUND_DOWN(usedBkInx, 8) % sb->s_bk_size] = rByte;
	bk_release(tmp_bk_cache);
//	//回写
