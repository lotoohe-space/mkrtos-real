...
#define __MIN_SMALL_SIZE	__SMALL_NR(128)		/*   16 /   32 /2 4*/
#define __MAX_SMALL_SIZE	__SMALL_NR(2)		/* 2048 / 4096 /128 256*/

#define GET_SIZE(s)		(__MIN_SMALL_SIZE<<get_index((s)))

#define FIRST_SMALL(p)		(((unsigned long)(p))&(~(MEM_BLOCK_SIZE-1)))

static inline int __ind_shift() { return (MEM_BLOCK_SIZE==256)?1:2; }
