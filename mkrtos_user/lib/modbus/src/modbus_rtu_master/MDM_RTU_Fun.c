...
#if !MD_STRICT_MODE
	pModbusRTU->frameIntervalTime = MD_FRAME_INTERVAL_TIME * 1000 / 10;
#else
...
void MDM_RTU_CB_Init(
	PModbus_RTU_CB pModbusRTUCB, PModbus_RTU pModbusRTU, uint32 sendIntervalTime, uint32 sendOverTime, uint8 RTTimes)
{
	...
	for (i = 0; i < MDM_REG_COIL_ITEM_NUM; i++)
	{
			...
			(pModbus_RTU->pMapTableList[i]->modbusAddr + pModbus_RTU->pMapTableList[i]->modbusDataSize) >= (modbusAddr + numOf))
		{
			if (pModbus_RTU->pMapTableList[i]->addrType == opAddrType)
			{
				...
				for (j = 0; j < numOf; j++)
				{
					MDS_RTU_REG_COIL_ITEM_DATA(pModbus_RTU->pMapTableList[i])
					[offsetAddr + j] =
	...
	void *wData)
{
	...
	if (MD_CB_GET_DIS_FLAG_EN(pModbus_RTU_CB) /*Enable the drop-off non-polling flag*/
		&& MD_CB_GET_DIS_FLAG(pModbus_RTU_CB))
	{ /*Device dropped*/
		return ERR_DEV_DIS;
	}
	...
	{ /*Has not been sent, or has been sent successfully*/
	...
	{ /*It has been sent, and there is no sending error*/
		...
		{ /*Data received*/
			...
			case 0x81:
				...
				goto _exit; /*Abnormal reading coil*/
			case 0x82:
				pModbus_RTU_CB->sendFlag = 3;
				errRes = ERR_READ_INPUT;
				goto _exit; /*Read input discrete quantity abnormal*/
			case 0x83:
				pModbus_RTU_CB->sendFlag = 3;
				errRes = ERR_READ_HOLD_REG;
				goto _exit; /*Read holding register error*/
			case 0x84:
				pModbus_RTU_CB->sendFlag = 3;
				errRes = ERR_READ_INPUT_REG;
				goto _exit; /*Error reading input register*/
			case 0x85:
				pModbus_RTU_CB->sendFlag = 3;
				errRes = ERR_WRITE_COIL;
				goto _exit; /*Write single coil error*/
			case 0x86:
				pModbus_RTU_CB->sendFlag = 3;
				errRes = ERR_WRITE_REG;
				goto _exit; /*Error writing order*/
			case 0x8F:
				pModbus_RTU_CB->sendFlag = 3;
				errRes = ERR_WRITE_COIL;
				goto _exit; /*Write multiple coil error*/
			case 0x90:
				pModbus_RTU_CB->sendFlag = 3;
				errRes = ERR_WRITE_REG;
				goto _exit; /*Write multiple register error*/
			}
		}
		if (pModbus_RTU_CB->sendFlag == 2)
		{ /*If the sending is successful, send again*/
			/*Timing resend processing*/
			if (pModbus_RTU_CB->pModbus_RTU->timesTick - pModbus_RTU_CB->sendTimeTick >= pModbus_RTU_CB->sendIntervalTime)
			{
				/*Resend*/
		...
		{ /*�Ѿ������ˣ����ǻ�û�յ���������ִ�г�ʱ���*/
			...
			if (pModbus_RTU_CB->pModbus_RTU->timesTick - pModbus_RTU_CB->sendTimeTick >= pModbus_RTU_CB->sendOverTime)
			{
				...
				if (pModbus_RTU_CB->RTCount >= pModbus_RTU_CB->RTTimes)
				{
					...
					if (MD_CB_GET_DIS_FLAG_EN(pModbus_RTU_CB))
					{										/*No polling when disconnected is enabled*/
	...
	{ /*If it has been sent, it will return the sending completion flag*/
		return ERR_SEND_FIN;
	}
_exit:
	//	MD_CLR_SENDED_FLAG(pModbus_RTU_CB);
...
MDError MDM_RTU_MixWriteSingleReg(
	PModbus_RTU_CB pModbus_RTU_CB, uint8 slaveAddr, uint16 startAddr, uint16 val)
{
	return MDM_RTU_RW_MIX(pModbus_RTU_CB, WRITE_SIN_REG, slaveAddr, startAddr, 1, (void *)(&val));
}
