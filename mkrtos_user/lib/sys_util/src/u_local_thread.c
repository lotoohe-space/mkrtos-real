...
int thread_create(int prio, void *(*fn)(void *arg), umword_t stack_top, obj_handler_t *ret_hd)
{
    ...
    msg_tag_t tag = u_factory_create_thread(FACTORY_PROT, vpage_create_raw3(KOBJ_ALL_RIGHTS, 0, th1_hd));
    if (msg_tag_get_prot(tag) < 0)
    {
        return msg_tag_get_prot(tag);
    }
    tag = u_thread_exec_regs(th1_hd, (umword_t)fn, stack_top, TASK_RAM_BASE(), 0);
    if (msg_tag_get_prot(tag) < 0)
    {
