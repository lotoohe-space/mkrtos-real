...
#else
	return be_futex(addr, op, val, to, 0, 0);
#endif
}
static int pthread_mutex_timedlock_pi(pthread_mutex_t *restrict m, const struct timespec *restrict at)
{
	...
	do
		e = -__futex4(&m->_m_lock, FUTEX_LOCK_PI | priv, 0, at);
	while (e == EINTR);
	if (e)
...
#else
			be_futex(&m->_m_lock, FUTEX_UNLOCK_PI | priv, 0, 0, 0, 0);
#endif
			self->robust_list.pending = 0;
			break;
		}
	...
	do
		e = __timedwait(&(int){0}, 0, CLOCK_REALTIME, at, 1);
	while (e != ETIMEDOUT);
	return e;
}
