...
void shellInit(Shell *shell, char *buffer, unsigned short size)
{
    ...
    for (int i = 0; i < shell->commandList.count; i++)
    {
        ...
        {
            cmd->data.cmd.desc = cmd->data.cmd.desc + start_addr;
            cmd->data.cmd.function = (int (*)())((unsigned long)cmd->data.cmd.function + start_addr | 0x1);
            cmd->data.cmd.name = cmd->data.cmd.name + start_addr;
        }
        else if (cmd->attr.attrs.type <= SHELL_TYPE_VAR_NODE)
        {
            cmd->data.var.desc += start_addr;
            cmd->data.var.value += start_addr;
            cmd->data.var.name += start_addr;
        }
        else if (cmd->attr.attrs.type <= SHELL_TYPE_USER)
        {
            cmd->data.user.desc += start_addr;
            cmd->data.user.password += start_addr;
            cmd->data.user.name += start_addr;
        }
