 ...
 * 通过文件名查找文件
 */
const dir_info_t *appfs_tiny_find_file_by_name(fs_info_t *info, const char *name)
{
    assert(info);
    assert(name);
...
fs_info_t *appfs_tiny_get_form_addr(void *addr)
{
    assert(addr);
    fs_info_t *fs = addr;
...
unsigned long appfs_tiny_find_file_addr_by_name(fs_info_t *info, const char *name, unsigned long *size)
{
    const dir_info_t *dir_info;

    dir_info = appfs_tiny_find_file_by_name(info, name);
    ...
    return (unsigned long)appfs_tiny_get_file_addr(info, dir_info);
}