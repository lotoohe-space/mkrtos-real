...
#define rt_used __attribute__((used))
#define rt_align(n) __attribute__((aligned(n)))
#define rt_weak weak //__attribute__((weak))
...
#define RT_TIMER_SKIP_LIST_MASK 0x3 /**< Timer skips the list mask */
    ...
    struct rt_timer
    {
        void (*func_cb)(void *arg); //!< 回调函数
        size_t start_times;         //!< 开始时间
        size_t exp_times;           //!< 期待值
        void *data;                 //!< 回调数据
        int inx;                    //!< 索引
        uint8_t flags;              //!< flags
    };
...
#define rt_get_thread_struct(object)         \
    do                                       \
    {                                        \
        rt_atomic_inc(&(object)->ref_count); \
    } while (0)
#define rt_put_thread_struct(object)         \
    do                                       \
    {                                        \
        rt_atomic_dec(&(object)->ref_count); \
    } while (0)
