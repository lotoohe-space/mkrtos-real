...
static void colon(char *buf)
{
    ...
    if (strncmp(p, "write", cnt) == 0 || strncmp(p, "wq", cnt) == 0 || strncmp(p, "wn", cnt) == 0 || (p[0] == 'x' && !p[1]))
    {
        ...
        else
        {
            ...
            if (p[0] == 'x' || p[1] == 'q' || p[1] == 'n' || p[0] == 'X' || p[1] == 'Q' || p[1] == 'N')
            {
                editing = 0;
            }
        }
    ...
    *buf1 = '\0';
    ...
    { // Edit a file
        ...
        li = count_lines(text, end - 1);
        status_line("'%s'%s" IF_FEATURE_VI_READONLY("%s") " %uL, %uC",
                    current_filename,
                    (size < 0 ? " [New file]" : ""),
    ...
    { // read file into text[]
        ...
        status_line("'%s'" IF_FEATURE_VI_READONLY("%s") " %uL, %uC",
                    fn,
...
key_cmd_mode:
    ...
    case '}': // }- move forward paragraph
        do
        {
            ...
            { // blank line not found, move to end of file
