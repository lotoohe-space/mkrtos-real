 ...
 * V1.1 20220531
 * 1, ????????
 * 2, ???u8/u16/u32?uint8_t/uint16_t/uint32_t
 ****************************************************************************************************
 */
...
uint8 *PPU_patterntables;       //8192  VROM?????? ?????
...
void PPU_reset(void)
{
    ...
    ppu->PPU_tile_tables = VROM_tiles; //????????
    ...
    if (RomHeader->flags_1 & 0x01)set_mirroring(0, 1, 0, 1); //???????	//  PPU_set_mirroring(); ???????
    else set_mirroring(0, 0, 1, 1); //??????
...
void scanline_draw(int LineNo)
{
  ...
  for (i = sx; i < ex; i++)
  {
    *(volatile uint16_t *)(info.display_addr) = NES_Palette[ppu->dummy_buffer[i++]];
    *(volatile uint16_t *)(info.display_addr) = NES_Palette[ppu->dummy_buffer[i++]];
    *(volatile uint16_t *)(info.display_addr) = NES_Palette[ppu->dummy_buffer[i++]];
    *(volatile uint16_t *)(info.display_addr) = NES_Palette[ppu->dummy_buffer[i++]];
    *(volatile uint16_t *)(info.display_addr) = NES_Palette[ppu->dummy_buffer[i++]];
...
void write_2007(uint8 data)
{
    ...
    if (addr >= 0x3000)
    {
        ...
        if (addr >= 0x3F00)
        {
            ...
            if (0x0000 == (addr & 0x000F)) // is it THE 0 entry?0???????
            {
...
#define NEW_BG_PIXEL() \
    ...
    *p = ppu->bg_pal[col]; \
    ...
    void render_bg(uint8 *buf)
    {
        ...
        uint32 tile_x; // pixel coords within nametable??????????nametable
        uint32 tile_y;
        uint32 name_addr;
        uint32 line;
        uint32 pattern_addr;
    ...
    tile_x = (ppu->loopy_v & 0x001F);
        ...
        for (i = 33; i; i--)
        {
            ...
            // are we crossing a dual-tile boundary???????dual-tile??????
        ...
        //  if(bg_clip_left8())
        //  {
        //    // clip left 8 pixels????8??????SIDE_MARGIN = 8,
    ...
    spr_height = sprites_8x16() ? 16 : 8;
        ...
        for (s = 0, spr = spr_ram; s < 64; s++, spr += 4)
        {
            ...
            for (x = start_x; x != end_x; x += inc_x)
            {
                ...
                // if a sprite has drawn on this pixel, don't draw anything??????????????????????,?????�ʦ�???
