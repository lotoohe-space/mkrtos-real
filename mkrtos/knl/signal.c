...
static inline int get_bit(unsigned long bits[_NSIG_WORDS],int inx){
    return (bits[inx/32]>>(inx%32))&0x1;
}
static inline void set_bit(unsigned long bits[_NSIG_WORDS],int inx,int status){
    if(status){
...
int32_t do_signal(void *cur_psp,uint32_t signr){
    ...
    //先复位信号
    set_bit(sys_tasks_info.current_task->sig_bmp,signr-1,0);
