...
extern int __fork_exec_shell(const char *file, char *const argv[]);
int fork_execvp(const char *file, char *const argv[]) {
  const char *path=getenv("PATH");
  char *cur,*next;
  char buf[PATH_MAX];
  int ret = -1;
  ...
  if (strchr((char*)file,'/')) {
	ret=fork_exec(file,argv,environ);
    if (ret >= 0) {
    	return ret;
    } else {
    	 if (errno==ENOEXEC)
    		 return __exec_shell(file,argv);
    }
  }
  if (!path) path=_PATH_DEFPATH;
  for (cur=(char*)path; cur; cur=next) {
    ...
    ret = fork_exec(buf,argv,environ);
    if (ret >= 0) {
    	return ret;
    } else {
		return __fork_exec_shell(buf,argv);
    }
    if (*next==0) break;
    next++;
  }
