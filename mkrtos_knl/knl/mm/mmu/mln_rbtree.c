...
/*rbtree_init*/
MLN_FUNC(, mln_rbtree_t *, mln_rbtree_init,
         (struct mln_rbtree_attr * attr, mln_rbtree_t *t), (attr, t),
         {
             if (t == NULL)
                 return NULL;
             if (attr == NULL)
...
MLN_FUNC_VOID(, void, mln_rbtree_insert, (mln_rbtree_t * t, mln_rbtree_node_t *node), (t, node), {
    ...
    if (y == nil)
        t->root = node;
    else if (t->cmp(node->data, y->data) < 0)
        y->left = node;
    else
        y->right = node;
    ...
    if (t->min == nil)
        t->min = node;
    else if (t->cmp(node->data, t->min->data) < 0)
        t->min = node;
    ++(t->nr_node);
...
MLN_FUNC_VOID(, void, mln_rbtree_delete, (mln_rbtree_t * t, mln_rbtree_node_t *n), (t, n), {
    ...
    if (y_original_color == M_RB_BLACK)
