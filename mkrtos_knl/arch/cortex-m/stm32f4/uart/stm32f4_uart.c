...
void uart_init(void)
{
    ...
    GPIO_InitTypeDef USART1_GPIO_InitStru;

    RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, ENABLE); // USART1的时钟
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);  // GPIOA的时钟

    USART1_GPIO_InitStru.GPIO_Pin = GPIO_Pin_9 | GPIO_Pin_10;
    USART1_GPIO_InitStru.GPIO_Mode = GPIO_Mode_AF;
    USART1_GPIO_InitStru.GPIO_Speed = GPIO_Fast_Speed;
    USART1_GPIO_InitStru.GPIO_OType = GPIO_OType_PP;
    USART1_GPIO_InitStru.GPIO_PuPd = GPIO_PuPd_UP;

    GPIO_PinAFConfig(GPIOA, GPIO_PinSource9, GPIO_AF_USART1);  // 映射为USART1的TX
    GPIO_PinAFConfig(GPIOA, GPIO_PinSource10, GPIO_AF_USART1); // 映射为USART1的RX

    GPIO_Init(GPIOA, &USART1_GPIO_InitStru);

    USART_InitTypeDef USART1_USART_InitStru;

    USART1_USART_InitStru.USART_BaudRate = uart.baud;
    USART1_USART_InitStru.USART_WordLength = USART_WordLength_8b;
    USART1_USART_InitStru.USART_StopBits = USART_StopBits_1;
    USART1_USART_InitStru.USART_Parity = USART_Parity_No;
    USART1_USART_InitStru.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;
    USART1_USART_InitStru.USART_HardwareFlowControl = USART_HardwareFlowControl_None;

    USART_Init(USART1, &USART1_USART_InitStru);

    NVIC_InitTypeDef USART1_NVIC_InitStru;

    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);

    USART1_NVIC_InitStru.NVIC_IRQChannel = USART1_IRQn;
    USART1_NVIC_InitStru.NVIC_IRQChannelPreemptionPriority = 0;
    USART1_NVIC_InitStru.NVIC_IRQChannelSubPriority = 0;
    USART1_NVIC_InitStru.NVIC_IRQChannelCmd = ENABLE;
